...
def FontSize(text,space,multi_ligne=False,max_font_size=None):
    ...
    current_size=(min(width,height) if not(max_font_size) else min(width,height,max_font_size) )+1
...
def write_text(picture,pos,text,text_colours,allignment='L',multi_line=False,max_font_size=None):

    Font_sise,word_sizes,Ligne_nbr,ligne_heights=FontSize(text,pos,multi_line,max_font_size)
    ...
    text=text.split(' ')
    ligne_center_offset = (abs(pos[3]-pos[1])-ligne_heights*Ligne_nbr)//2 * (allignment=='CC' or allignment=='LC') # avilable space - nbr_lignes*line_ize //2 and 0 if allignment !=0
    ligne_end_index=-1
    current_word=0
    for current_line in range(Ligne_nbr):
        ligne_end_index+=1

        center_offset=(abs(pos[2]-pos[0])-ligne_lenghts[current_line])//2 * (allignment=='CC')# avilabe space - needed space /2 and 0 if allignment !=c
